...
/* Begin hhcho code */
RandomStream::RandomStream(const unsigned char *data, bool serialized)
{
   if (serialized) {
     memcpy(state, data, 16 * sizeof(_ntl_uint32));
     memcpy(buf, data + 16 * sizeof(_ntl_uint32), 64);
     memcpy(&pos, data + 16 * sizeof(_ntl_uint32) + 64, sizeof(long));
   } else {
     salsa20_init(state, data);
     pos = 64;
   }
}

void RandomStream::serialize(unsigned char *data)
{
   memcpy(data, state, 16 * sizeof(_ntl_uint32));
   data += 16 * sizeof(_ntl_uint32);
   memcpy(data, buf, 64);
   data += 64;
   *((long *) data) = pos;
}
